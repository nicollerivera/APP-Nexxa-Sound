...
function App() {
  ...
  const [selectedPackageId, setSelectedPackageId] = useLocalStorage('nexxa_pkg_v2', null);
  const [activeExtras, setActiveExtras] = useLocalStorage('nexxa_extras_v2', {});
  const [guestCount, setGuestCount] = useLocalStorage('nexxa_guests_v2', 10);

  const [clientName, setClientName] = useLocalStorage('nexxa_client_v2', '');
  const [eventDate, setEventDate] = useLocalStorage('nexxa_date_v2', '');
  const [eventStartTime, setEventStartTime] = useLocalStorage('nexxa_start_v2', '');
  const [eventEndTime, setEventEndTime] = useLocalStorage('nexxa_end_v2', '');
  const [startAmPm, setStartAmPm] = useLocalStorage('nexxa_start_ampm_v2', 'PM');
  const [endAmPm, setEndAmPm] = useLocalStorage('nexxa_end_ampm_v2', 'AM');
  const [eventNeighborhood, setEventNeighborhood] = useLocalStorage('nexxa_neighborhood_v2', '');
  const [eventAddress, setEventAddress] = useLocalStorage('nexxa_address_v2', '');
  ...
  const [makeupCount, setMakeupCount] = useLocalStorage('nexxa_makeup_v2', 1);
        ...
        <div className="container summary-content">
          <div className="total-container">
            <span
              ...
              onClick={scrollToTop}
              style={{ cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '5px', color: '#00d4ff', marginBottom: '4px' }}
            >
          ...
          <div style={{ position: 'absolute', bottom: '2px', right: '5px', fontSize: '0.8rem', color: '#ff0055', fontWeight: 'bold' }}>v1.8</div>
        </div>
      </footer>
    </div>
  );
